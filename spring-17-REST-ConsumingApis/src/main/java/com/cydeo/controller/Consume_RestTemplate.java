...
public class Consume_RestTemplate {
    ...
    //URI of json file that we want to consume
    ...
    @GetMapping
    //getForEntity method returns responseEntity(receives the uri and clas),output is mapped to dto
    public ResponseEntity<User[]> readAllUsers(){
        return restTemplate.getForEntity(URI, User[].class);
    }

    @GetMapping("{id}")
    //in this method we don't use the dto class to get the user, just showing the json result
